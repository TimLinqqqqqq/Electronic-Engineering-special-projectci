...
struct local *find_location(struct ssd_info *ssd,unsigned int ppn)
{
     ...
     ********************************************************************************/
    if(ssd->pre_process = 1)
	location->channel = ppn/page_channel;
    else if(ssd->pre_process = 0)
    	location->channel = ssd->request_tail->channel_assign;
    else
	printf("Error of pre_process!");
...
unsigned int get_new_block_LRU(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane){
    ...
    for(i=0;i<ssd->parameter->block_plane;i++){
        if(ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[i].last_write_page == -1){
            block = i;
			#ifdef MPDEBUG
