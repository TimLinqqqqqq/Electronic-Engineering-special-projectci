...
struct ssd_info{ 
    char buffer_collect[80000][200];
    char buffer_output[80000][200];
    int flag5000;
    int pre_process;			 // 1 as pre_processing, 0 as not.
    int64_t final_t;
...
struct feature_info{
    int level;
    unsigned int R_counts;
    unsigned int W_counts;
    unsigned int R_ch; // How many channels for READ
    unsigned int W_ch; // How many channels for WRITE
    int tenant_mode;	// mode 0: 2 tenants sharing 8 channels. mode 1: 4 tenants sharing 8 channels. 
    char tenant_buffer[2][20]; //tenant buffer[tenant_mode][W_ch R_ch].
    unsigned int workload0;
    unsigned int workload1;
    unsigned int RW0_ch;
    unsigned int RW1_ch;
    unsigned int WW0_ch;
    unsigned int WW1_ch;
...
struct request{
    ...
    unsigned int channel_assign;	//請求該放置在第幾個channel（0~7）
...
struct feature_info *initialize_feature(struct ssd_info * ssd);
