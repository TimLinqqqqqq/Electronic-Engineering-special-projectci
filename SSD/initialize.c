...
struct ssd_info *initiation(struct ssd_info *ssd)
{
    ...
    //初始化 FCL
    ssd->feature=(struct feature_info*)malloc(sizeof(struct feature_info));
    alloc_assert(ssd->feature,"ssd->feature");
    memset(ssd->feature,0,sizeof(struct feature_info));
    initialize_feature(ssd);
...
{
    struct feature_info *feat=ssd->feature;
    feat->level = 0;
    feat->R_counts = 0;
    feat->W_counts = 0;
    feat->R_ch = 10;
